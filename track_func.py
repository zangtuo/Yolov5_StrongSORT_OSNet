...
'''监控设备'''
def cam(source='rtsp://11.11.11.11', h=1024, w=960):
    pass


'''图像分析'''
def img(source='img.jpeg', h=1024, w=960):
    pass

'''视频分析'''
def vid(source='vid.mp4', h=1024, w=960):
    trace = [];
    '''read video'''
    ...
    for m in range(grid_w):
        for n in range(grid_h):
            '''find all velo in this area & neighbors'''
            velo = [0, 1, 2]
            v_avg = [0,0]
            for v in velo:
                v_avg += v[1:]
            v_avg = np.average(np.array(v_avg)[1:,:]).toarray()
